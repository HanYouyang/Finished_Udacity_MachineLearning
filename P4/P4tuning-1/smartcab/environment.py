...
class Environment(object):
    ...
    def reset(self):
        ...
        leastdistance = self.compute_dist(start, destination)
        print "Environment.reset(): Trial set up with start = {}, destination = {}, deadline = {}".format(start, destination, deadline)
        print "At least the distance is {}.".format(leastdistance)
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                if state['deadline'] >= 0:
                    reward += 10  # bonus
                self.done = True
                print "Environment.act(): Primary agent has reached destination \nwith time step for {},\n'final' reward is {}.".format(self.t, reward) # [debug]
                dumblevel = self.t / leastdistance
                print "Environment.act(): The final wisdom of this trail is {}.".format(dumblevel)
